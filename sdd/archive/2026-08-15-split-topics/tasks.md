...
- [x] 13. 使用者主動調整：`src/routes/topics/[slug]/+page.svelte` 移除任務 6 保留下來的返回按鈕，此為提交後使用者手動變更，非 AI 遺漏；與任務 6、驗收條件第 6 點所述「保留返回按鈕」的原始規劃不一致，記錄於此供後續參考
...
- 情境：造訪 `/topics/[slug]` 頁面，不再看到重複的主題清單區塊，只看到返回按鈕、主題標題與該主題的文章列表（後續使用者主動移除返回按鈕，見任務 13，此情境現況已不成立）
