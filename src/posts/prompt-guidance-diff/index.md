...
- 對 GPT 只有在流程真的重要時才指定步驟，例如稽核、法規檢查、資料清理、測試驗證。
- 對 Claude 當順序、完整性、輸出一致性重要時，有序的方式來規劃流程是合理做法。Anthropic 也建議在順序或完整性重要時，使用條列或編號步驟。

### 兩家都開始重視「控制思考成本」

- OpenAI 文件也提到在把問題升級成「高推理成本（high effort）」之前，應該先重新檢查低成本（low）和中等成本（medium）的方法是不是已經足夠解決問題。
- Claude 也有推理成本設定，官方提醒**高推理成本可能帶來更高 token 成本**，甚至過度思考；如果複雜任務推理太淺，才提高推理成本或加入針對性提示。

🔥 **在提高推理成本之前，應先釐清目標並用低到中等推理嘗試解決；只有當結果不能滿足需求時，才逐步升級推理層級，而不是讓 AI 一開始就對模糊問題進行高成本推理**。

以模型選擇舉例：

- 在已知的晚餐選項（例如：牛排、沙拉、義大利麵）中選擇今天的菜單，可能只需要低推理成本，甚至是不用思考的 GPT-4.1 就能勝任。
- 在需要分析複雜數據或進行多步推理的任務中（例如：尋找某一個區域的所有餐廳評價，以決定最佳用餐地點），可能才需要中等到高推理成本的模型，如 GPT-5.5。
