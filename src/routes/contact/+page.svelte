...
<div class="flex min-h-screen w-full flex-col md:flex-row bg-background text-foreground">
		...
		<div class="fixed inset-0 z-50 bg-background flex flex-col p-6 md:hidden animate-in slide-in-from-left duration-300">
			...
			<nav class="flex flex-col text-lg">
				<div class="flex flex-col gap-4">
					...
					<a href="/contact" class="font-medium text-foreground hover:text-primary transition-colors" onclick={toggleMenu}>Contact</a>
				</div>
	...
	<nav class="hidden md:flex w-full md:w-80 md:sticky md:top-0 md:h-screen z-40 border-b md:border-b-0 md:border-r bg-background/95 backdrop-blur p-8 flex-col justify-between">
		<div class="space-y-6">
			...
			<div class="flex flex-col gap-3 pt-2">
				...
				<a href="/contact" class="text-sm font-medium text-foreground hover:text-primary transition-colors">Contact</a>
			</div>
		</div>
	...
	<main class="flex-1">
		<div class="container max-w-4xl mx-auto px-6 py-8 md:py-12 space-y-12">
			...
			<section class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
				...
				<div class="max-w-2xl space-y-6">
					<p class="text-lg text-muted-foreground leading-relaxed">
						Have a project in mind, a question, or just want to discuss the latest in AI and design? I’d love to hear from you.
					</p>
					<p class="text-lg text-muted-foreground leading-relaxed">
			...
			<!-- Contact Form -->
			<section class="max-w-xl animate-in fade-in slide-in-from-bottom-4 duration-700 delay-100 w-full">
				<form class="space-y-8">
					<!-- Row 1: Inputs -->
					<div class="flex flex-row gap-6">
						<div class="flex flex-col gap-2 w-full">
							<label for="name" class="text-sm font-bold">Name</label>
							<Input id="name" placeholder="Akim Perminov" class="bg-zinc-950 border-zinc-800 text-white placeholder:text-zinc-600 h-11 rounded-md focus-visible:ring-0 focus-visible:bg-zinc-900 transition-colors duration-300 py-2 px-3" />
						</div>
						<div class="flex flex-col gap-2 w-full">
							<label for="topic" class="text-sm font-bold">Topic</label>
							<Input id="topic" placeholder="Guest post" class="bg-zinc-950 border-zinc-800 text-white placeholder:text-zinc-600 h-11 rounded-md focus-visible:ring-0 focus-visible:bg-zinc-900 transition-colors duration-300 py-2 px-3" />
						</div>
					</div>
					...
					<div class="flex flex-row gap-6">
						<div class="flex flex-col gap-2 w-full">
							...
							<textarea id="message" placeholder="Your message" class="flex w-full bg-zinc-950 border border-zinc-800 text-white placeholder:text-zinc-600 outline-none min-h-[160px] rounded-md resize-none py-2 px-3 text-sm focus-visible:ring-0 focus-visible:border-ring transition-colors duration-300"></textarea>
						</div>
					</div>

					<!-- Row 3: Submit -->
					<div class="flex flex-row gap-6">
						<Button type="submit" size="lg" class="w-full bg-white! text-black! hover:bg-zinc-200! font-medium rounded-md h-12 cursor-pointer">
							Submit
						</Button>
					</div>
				</form>
