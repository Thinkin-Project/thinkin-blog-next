...
<div class="flex min-h-screen w-full flex-col md:flex-row bg-background text-foreground">
		...
		<div class="fixed inset-0 z-50 bg-background flex flex-col p-6 md:hidden animate-in slide-in-from-left duration-300">
			...
			<nav class="flex flex-col text-lg">
				...
				<div class="space-y-4">
					...
					{#each links as link}
						<a href={link.url} class="flex items-center gap-4 font-medium text-muted-foreground hover:text-foreground transition-colors group" onclick={toggleMenu}>
	...
	<nav class="hidden md:flex w-full md:w-80 md:sticky md:top-0 md:h-screen z-40 border-b md:border-b-0 md:border-r bg-background/95 backdrop-blur p-8 flex-col justify-between">
		...
		<div class="mt-8 md:mt-auto pt-6 space-y-4">
			...
			<p class="text-xs text-muted-foreground">© 2025 Frank Price</p>
		</div>
	</nav>
	...
	<main class="flex-1">
		<div class="container max-w-4xl mx-auto px-6 py-8 md:py-12 space-y-12">
					...
					{#each featuredPosts as post}
						<a href={post.link} class="block group relative bg-zinc-900 hover:bg-zinc-800 transition-colors duration-300 p-4">
							<div class="flex items-center justify-between z-10 relative">
								...
								<ArrowRight class="h-5 w-5 text-zinc-600 group-hover:text-white transition-transform group-hover:translate-x-1" />
							</div>
						</a>
					{/each}
				</div>
						...
						<li class="group">
							<a href="#" class="flex flex-col md:flex-row md:items-center justify-between py-3 group-hover:bg-zinc-900/50 rounded-lg px-3 transition-colors -mx-3">
								<div class="flex items-center gap-3 relative">
									<ArrowRight class="h-5 w-5 text-primary opacity-0 -translate-x-4 absolute left-0 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-500 ease-out" />
									<div class="space-y-1 transition-transform duration-500 ease-out group-hover:translate-x-8">
			...
			<!-- Footer Section -->
			<footer class="pt-8 border-t border-border space-y-8 pb-8">
